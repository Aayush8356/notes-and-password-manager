...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const allPassword = await Password.find({ user: req.user.id }).sort({
        date: -1,
      });
...
router.get("/", auth, async (req, res) => {
  ...
  try {
    var allPassword = await Password.find({ user: req.user.id }).sort({
      date: -1,
    });
    allPassword.map((passwd) =>
      alldecyptedPassword.unshift({
